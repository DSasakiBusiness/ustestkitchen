...
// Custom Line-drawing SVGs for specific foods
...
const Sushi = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
const Ramen = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
const Steak = ({ className }: { className?: string }) => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
const foodIcons = [Hamburger, Sushi, Ramen, Pizza, Drink, Steak];

export default function FoodBackground() {
  const [items, setItems] = useState<{ id: number; Icon: any; x: number; y: number; rotate: number; scale: number; duration: number }[]>([]);
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      {items.map((item) => (
        <motion.div
            key={item.id}
            className="absolute text-white/10 opacity-20 pointer-events-none" // Changed to white for dark background visibility
            initial={{ 
                left: `${item.x}%`, 
                top: `${item.y}%`, 
                rotate: item.rotate,
            ...
            animate={{ 
                y: [0, -50, 0], 
                rotate: [item.rotate, item.rotate + 20, item.rotate - 20],
                opacity: [0.1, 0.3, 0.1]
            }}
            transition={{ 
